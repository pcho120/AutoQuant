...
import type { MarketPayload } from './App'
...
const chartOptions = {
  ...
  timeScale: { borderColor: '#d9dde3', timeVisible: true, secondsVisible: false },
  crosshair: { vertLine: { color: '#708090' }, horzLine: { color: '#708090' } },
}

function ChartHost({ label, height, render }: ChartHostProps) {
  const containerRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (!containerRef.current) return
    const container = containerRef.current
    const chart = createChart(container, { ...chartOptions, height, width: container.clientWidth })
    render(chart)
...
export const MarketCharts = memo(function MarketCharts({ data }: { data: MarketPayload }) {
  return (
    <div className="chart-stack">
      <ChartHost label="PRICE · CANDLESTICK" height={330} render={(chart) => {
      ...
      <ChartHost label="VOLUME" height={120} render={(chart) => {
      ...
      <ChartHost label="RSI (14)" height={150} render={(chart) => {
      ...
      <ChartHost label="MACD (12, 26, 9)" height={170} render={(chart) => {
